...
app.get("/processing_task/:user_id", async function(req, res) {
    var user_id = req.params.user_id;

    var sql = `SELECT * FROM tasks WHERE task_status = 'In Progress' AND user_id = ?`;
    var processing_task = await exe(sql, [user_id]);

    var sql2 = `SELECT * FROM users WHERE user_id = ?`;
    var user_info = await exe(sql2, [user_id]);

    var packet = {processing_task, user_info};
    res.render("processing_task.ejs", packet); 
});



app.get("/completed_task/:user_id", async function(req, res) {
    var user_id = req.params.user_id;

    var sql = `SELECT * FROM tasks WHERE task_status = 'Completed' AND user_id = ?`;
    var completed_task = await exe(sql, [user_id]);

    var sql2 = `SELECT * FROM users WHERE user_id = ?`;
    var user_info = await exe(sql2, [user_id]);

    var packet = {completed_task, user_info};
    res.render("completed_task.ejs", packet); 
});
